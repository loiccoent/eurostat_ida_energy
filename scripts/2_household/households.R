...
# Data preparation
household_final <- function(first_year,
                            last_year,
                            country,
                            data_path,
                            chart_path) {
  ...
  HH_energy_breakdown <-  nrg_bal_c %>%
    ...
    mutate(product = factor(product, level = IDA_HH_PROD)) %>%
    group_by(geo, time) %>%
  ...
  energy_HH_aggregated$total_bal <-
    replace(energy_HH_aggregated$total_bal,
  ...
  energy_HH_disaggregated$cooking <-
    replace(energy_HH_disaggregated$cooking,
  ...
  energy_HH_disaggregated$other <-
    replace(energy_HH_disaggregated$other,
  ...
  pop$total_pop <-
    replace(pop$total_pop,
            which(pop$total_pop <= 0),
            NA)
  size_HH$HH_size <-
    replace(size_HH$HH_size,
            which(size_HH$HH_size <= 0),
            NA)
  CHDD$CDD_norm <-
    replace(CHDD$CDD_norm,
            which(CHDD$CDD_norm <= 0),
            NA)
  CHDD$HDD_norm <-
    replace(CHDD$HDD_norm,
            which(CHDD$HDD_norm <= 0),
            NA)

  # joining datasets

  HH <- energy_HH_aggregated %>%
    full_join(energy_HH_disaggregated, by = c("geo", "time")) %>%
    full_join(pop, by = c("geo", "time")) %>%
  ...
  for (country_chart in countries) {
    ...
    first_year_chart <-
      household_base_year(country=country_chart, first_year = first_year)
    last_year_chart <-
      household_last_year(country = country_chart, final_year = last_year)
    ...
    p <- subsector_data %>%
      ...
      scale_color_manual(values = ColorsEndUse, limits = force) +
      theme_classic() +
      theme(
        axis.title.x = element_blank(),
        legend.title = element_blank(),
    ...
    write.csv(
      table_HH_energy_breakdown_filtered,
      paste0(outputpath, "Part4_fuel.csv"),
      row.names = FALSE
    )

    year <-
      as.Date(as.character(HH_energy_breakdown_filtered$time),
              "%Y")
    ...
    p <- HH_filtered_data %>%
      ...
      scale_fill_manual(values = ColorsEndUse, limits = force) +
      theme_classic() +
      theme(
        axis.title.x = element_blank(),
        legend.title = element_blank(),
    ...
    write.csv(table_HH_augmented,
              paste0(outputpath, "Part4_enduse.csv"),
              row.names = FALSE)
    ...
    write.csv(household_Waterfall_data,
              paste0(outputpath, "Part4_waterfall.csv"),
              row.names = FALSE)
    ...
    write.csv(HH_intensity_effect_data,
              paste0(outputpath, "Part4_intensity_effect.csv"),
              row.names = FALSE)
    ...
    write.csv(HH_augmented %>%
                filter(measure %in% c("HDD", "CDD", "HH_size"),
                       geo == country_chart),
              paste0(outputpath, "Part4_HDD_CDD_HH.csv"),
              row.names = FALSE)
  ...
  if (country == "EU27") {
    ...
    write.csv(household_intensity_EU_comparison_data,
      paste0(outputpath, "Part3_EU27.csv"),
      row.names = FALSE)
