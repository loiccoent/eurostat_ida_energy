...
industry_GVA_primary <- function(first_year,
  ...
  industry_GVA_primary_filtered <- industry_GVA_primary_complete %>% 
  ...
  print('MT full period Mining and quarrying, Wood and wood products, 
        Coke and ref. pet. products, Non-metallic minerals, 
        Transport equipment removed (missing GVA)')
  ...
  industry_GVA_primary_augmented <- industry_GVA_primary_filtered %>% 
    ...
    select(-c(total_primary_energy_consumption,
  ...
  for (country_chart in countries) {
    ...
    write.csv(table_ele_heat_share_primary_filtered, paste0(outputpath, "Part2_share.csv"), row.names = FALSE)
    
    table_ele_heat_input_breakdown_filtered <- ele_heat_input_breakdown_filtered %>%
      mutate(energy_input = round(energy_input, 1),
             share_energy_input = round(share_energy_input, 3)) 
    
    write.csv(table_ele_heat_input_breakdown_filtered, paste0(outputpath, "Part2_fuel.csv"), row.names = FALSE)
    ...
    write.csv(table_industry_GVA_primary_country_data, paste0(outputpath, "Part2_sector.csv"), row.names = FALSE)
    ...
    year <-
      as.Date(as.character(ele_heat_input_breakdown_filtered$time),
              "%Y")
  ...
  # chart indexed variation by subsector
    ...
    year <-
      as.Date(as.character(industry_GVA_primary_transformation$time),
              "%Y")
    ...
    min_EU <-
      apply(select_if(heat_input_comparison[, -1], is.numeric), 1, min, na.rm =
              TRUE)
    max_EU <-
      apply(select_if(heat_input_comparison[, -1], is.numeric), 1, max, na.rm =
              TRUE)
    avg_EU <-
      apply(select_if(heat_input_comparison[, -1], is.numeric), 1, mean, na.rm =
              TRUE)
    ...
    p <- ggplot(data = industry_GVA_primary_effects,
           aes(x = factor(time),
               y = value / 1000)) +
      geom_bar(aes(fill = Effect),
               stat = "identity")  +
      scale_fill_manual(values = ColorsEffect) +
      geom_point(data = industry_GVA_primary_results,
                 aes(y = value / 1000),
                 size = 3) +
      theme_classic() +
    ...
    industry_GVA_primary_Waterfall_data <-
      ...
      mutate(measure = case_when((x == !!Result_label) ~ "total",
                                 TRUE ~ "relative"))
      ...
      p <- industry_GVA_primary_intensity_effect %>%
        ...
        geom_point(data = (industry_GVA_primary_intensity_effect %>% 
                            filter(measure == "Without intensity and transformation effect",
      ...
      min_EU <-
       apply(select_if(industry_GVA_primary_transformation_comparison[, -1], is.numeric),
             1,
             min)
      max_EU <-
       apply(select_if(industry_GVA_primary_transformation_comparison[, -1], is.numeric),
      ...
      avg_EU <-
       apply(select_if(industry_GVA_primary_transformation_comparison[, -1], is.numeric),
             ...
             mean)
      year <-
       as.Date(as.character(industry_GVA_primary_transformation_comparison$time),
               "%Y")
      ...
      p <- industry_GVA_primary_transformation_comparison %>%
       ...
       ylab("Energy transformation efficiency") + 
        scale_x_continuous(breaks = c(first_year,round((first_year+last_year)/2), last_year)) +
        scale_y_continuous(labels = scales::number)
      # ggtitle(paste("Energy transformation efficiency in", country_name, "'s ", sector_chart, " subsector compared to \nother European countries"))
      ...
      industry_GVA_primary_efficiency_comparison_sector <-
       industry_GVA_primary_complete %>%
       mutate(sector = factor(sector, levels = IDA_IND_SECTOR)) %>%
        select(-c(final_energy_consumption, primary_energy_consumption,
      ...
      year <-
       as.Date(as.character(industry_GVA_primary_efficiency_comparison_sector$time),
               "%Y")
      ...
      p <- industry_GVA_primary_efficiency_comparison_sector %>%
       ...
       scale_x_continuous(breaks = c(first_year,round((first_year+last_year)/2), last_year)) +
        scale_y_continuous(labels = scales::number) +
       #(paste("Energy transformation efficiency in", country_name, "'s subsector compared to \nother European countries")) +
  ...
  if (country == "EU27"){
    ...
    p <- industry_GVA_primary_complete %>%
      ...
      select(c("geo", "time", "sector","primary_energy_consumption", "GVA")) %>%
       replace(is.na(.), 0) %>%
       mutate(missing =
                case_when((primary_energy_consumption > 0 & GVA > 0) | 
    ...
    EU_comparison <- energy_EHG_TJ %>%
      ...
      select(-c('geo', 'label')) %>% 
      pivot_longer(cols=c("E7000_INPUT", "H8000_INPUT"),
                   names_to = "measure",
                   values_to = "value")
